                    ...
                    for (let user of participants) {
                       let pp = './src/welcome.jpg'
                        try {
                            ...
                            text = (action === 'add' ? (chat.sWelcome || this.welcome || conn.welcome || 'Selamat datang kak👋, di grup').replace('@subject', groupMetadata.subject).replace('@desc', groupMetadata.desc.toString()) :
                                (chat.sBye || this.bye || conn.bye || 'Yang Keluar Jangan Lupa Beliin Gorengan 5 Ribu🐦'))
                                this.sendButtonImg(id, pp, text, "Sachi Botz ♡︎", "Oke Bot 😝", "wkwk", null)
                                }
                    }
                }
                break
