let handler = async (m, { conn, text }) => {
  let chats = conn.chats.map().filter(v => v.jid.endsWith('.net')).map(v => v.jid)
  ...
  for (let id of chats) {
    await delay(1500)
    await conn.copyNForward(id, conn.cMod(m.chat, cc, /bc|broadcast/i.test(teks) ? teks : '〔 *Sachi-Broadcast* 〕\n\n' + teks + '\n\n' + watermark), true).catch(_ => _)
  }
